...
// Лёгкий короткий "тик" — для любых мелких переключений: чипы фильтров,
// выбор темы, вкладки листа персонажа, чекбоксы владения, смена раздела
// в нижней навигации. Нарочно тише и суше playFormatClick, чтобы не
// надоедать при частых кликах.
...
// Короткий приятный перезвон на успешное действие: сохранение персонажа/
// существа/предмета/заклинания, добавление фото, импорт и т.п.
function playSuccessChime() {
  if (!soundsEnabled()) return;
  try {
    const ctx = getAudioCtx();
    const now = ctx.currentTime;
    const notes = [880, 1174.66]; // A5, D6 — короткий восходящий интервал
    notes.forEach((freq, i) => {
      const t = now + i * 0.07;
      const osc = ctx.createOscillator();
      osc.type = 'sine';
...
// Мягкий низкий сигнал для ошибок/предупреждений в тостах — не резкий,
// просто явно отличается от success-перезвона
