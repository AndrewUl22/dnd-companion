...
- Владение навыком можно поднять до Компетенции (двойной бонус мастерства) —
  клик по кружку у навыка циклит пусто → владение (✓) → компетенция (★)
- Блок "Безделушки и заметки" в Инвентаре — свободный текст для мелких
  вещей, которые не хочется заводить как отдельные предметы
- Предметы могут давать бонус к характеристике (например, кольцо +2 Силы) —
  настраивается в форме предмета, действует, пока предмет экипирован, и
  учитывается во всех расчётах (проверки, спасброски, навыки, КД,
  заклинательство, атаки), не только для отображения
